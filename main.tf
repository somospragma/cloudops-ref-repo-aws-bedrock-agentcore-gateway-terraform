...
resource "aws_bedrockagentcore_gateway_target" "this" {
  ...
  dynamic "metadata_configuration" {
    for_each = each.value.config.metadata_configuration != null ? [each.value.config.metadata_configuration] : []

    content {
      allowed_request_headers  = length(metadata_configuration.value.allowed_request_headers) > 0 ? metadata_configuration.value.allowed_request_headers : null
      allowed_response_headers = length(metadata_configuration.value.allowed_response_headers) > 0 ? metadata_configuration.value.allowed_response_headers : null
      allowed_query_parameters = length(metadata_configuration.value.allowed_query_parameters) > 0 ? metadata_configuration.value.allowed_query_parameters : null
    }
  }
